        ...
        if(node.nodeType===3){
          node.textContent.split(/(\s+)/).forEach(function(w){
            if(/^\s+$/.test(w)){frag.appendChild(document.createTextNode(w));}
              ...
              var outer=document.createElement('span');outer.className='word';outer.style.display='inline-block';outer.style.overflow='hidden';outer.style.paddingTop='0.1em';outer.style.paddingBottom='0.15em';outer.style.marginTop='-0.1em';outer.style.marginBottom='-0.15em';
